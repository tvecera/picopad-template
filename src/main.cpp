...
const char *const FirstMenuTitles[MENUNUM] = {
        "Ants",
        "Atoms",
        "Eggs",
        "Fifteen",
...
const char *const SecondMenuTitles[MENUNUM] = {
        "Empty",
        "Empty",
        "Empty",
        "Empty",
...
const pMenuFnc FirstMenuFnc[MENUNUM] = {
        namespace_ants::main,
...
const pMenuFnc SecondMenuFnc[MENUNUM] = {
        empty,
        empty,
        empty,
        empty,
...
void DrawTiles(const char * const *Tiles, int x, int y) {
    // draw text
    for (int i = 0; i < MENUNUM; i++) {
        y = (i / MENUW) * ICONH + TEXTY;
        x = (i % MENUW) * ICONW;
        x += (ICONW - StrLen(Tiles[i]) * 8) / 2;
        DrawText(Tiles[i], x, y, COL_WHITE);
    }
...
int main() {
    ...
    while (True) {
        ...
        if (page == 0) {
            ...
            DrawTiles(FirstMenuTitles, x, y);
        } else {
            // draw menu image
            DrawImgRle(Menu2Img, Menu2Img_Pal, 0, 0, WIDTH, HEIGHT);
            DrawTiles(SecondMenuTitles, x, y);
        }
